...
This guide will help you understand how to **install**, **remove**, and **update** packages on your ***TCET Linux system***. **TCET Linux** follows a package management system named **pacman**, making it *efficient*, *user-friendly* and *easy* to learn and operate. TCET Linux follows *rolling releases of applications and packages* hence packages are updated on regular basis hence it is important to *synchronize the package database* and *update the application*. Many times for *troubleshooting* and *system maintenance* you might need to remove a package, hence the guide will also help you there.

## Pacman
Packages in TCET Linux is managed by package manager called pacman. The official releases of packages are published on the pacman package manager. Here `sync`, `install`, `remove`, `autoremove` are the shortcut command that is `alias commands` as mentioned in [**Alias Commands Section**](alias-commands). In this commands, `sudo` is used to run the command with *superuser privileges*, allowing you to *install packages system-wide* and `pacman` is the name of the *package manger*.
...
The command `sudo pacman -Syyy` is used to *refresh the package databases* in TCET Linux, with a full refresh. *Here's the breakdown of this command*:
...
OR 
  ...
  The command `sudo pacman -S` is used to install packages from the **official repositories** of the in TCET Linux.
- `-S` Flag is used to specify that you want to install a package.
...
OR
   ...
   ```
   This command `sudo pacman -R` in TCET Linux, is used to **remove packages** from your system **without removing dependencies installed along with it**.
- `-R` This flag is used to specify that you want to remove a package.
...
OR
   ...
   ```
The `sudo pacman -Rns` command in TCET Linux, is used to *remove a package along with its dependencies* that are no longer required by any other installed packages. *Here's the breakdown of this command*:
- `-R` Flag is used to specify that you want to remove a package.
- `-n` Flag is used to remove only packages that are no longer required by any other installed packages (orphans).
- `-s` Flag is used to remove package dependencies as well.
...
### Installing YAY
To install the **yay AUR helper**, we need to install the `git` and `base-devel` group of package on the system. So to install the git and base-devel on the system, use the following command:
