...
17.	We also *add a new row* to the `tbl02` table. It contains two columns: one with the value of `c20` and another with an input field. It's like adding a new row to the table and filling it with values
 ...
 _We have successfully understood the [**Table Updater**](table-updater) function! Let's move on and understand the [**Table Row Updater**](table-row-updater) function of the **[Fill My Cycle](fillmycycle.tcetmumbai.in/)**._