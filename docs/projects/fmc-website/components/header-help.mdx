...
2. Inside the file we have firstly created and defined the head section giving it a name also added some tags like `<meta charset="utf-8">` tag tells the computer how to understand and show the characters on the page. Some other tags are The `<meta name="description" content="...">` tag is like a small description of what the page is about. It tells people and search engines a little bit about what they can expect to find on the page and the `<meta name="viewport" content="...">` tag helps make sure the page looks good on different devices like phones or tablets.

3. But in this section our prime focus is to obtain the fill my cycle logo this can be found later in the **index.html** code. Here we created different div classses for classifying different parts of codes and simply used html to type the logo enclosed under `<h3>` tag.
...
``` 
4. This looks something like this
...
Making it very necessary for us as well to put theme changer to make user experience better and memorable. Let's now understand it's code now:
...
4. We have used if-else statement here, if darkMode is equal to false, it means the dark mode is currently off. In that case, the darkModeProp() function is called. This function sets the style properties to define the dark mode appearance, updates the mode text to "LIGHT", changes the mode icon to a sun, and sets darkMode to true and uses the parameters given in the code. If darkMode is not equal to false it means dark mode is currently on. 

``` js title= "script.js" {9-23} showLineNumbers
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function darkModeProp() {
		...
		// It's followed by similar parameters
...
4. Overall, the code represents a row with a form that has a heading saying "Need Help?" and a button when the user clicks leads him to instruction part.
...
_The [**Header Section**](header) is succesfully executed! Let's move on and see how we created the [**Definig cycles**](defining-cycle) of the **TNP Website**._
