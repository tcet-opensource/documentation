...
function InstallationSteps() 
    ...
    //Futher Code
  ```
3. Our implemented `code` begins with importing the necessary dependencies. `React`and the **UseState Hook** are imported from the React library. We have also imports several image files using relative paths for our `Requirements`.
...
4. Similar Steps were carried out by us to create different object in both the **Arrays**.
...
<figure>
    <center><img src={set1} style={{ border: "2px solid gray", width: "400px" }} /></center>
...
<figure>
    <center><img src={set2} style={{ border: "2px solid gray", width: "400px" }} /></center>
