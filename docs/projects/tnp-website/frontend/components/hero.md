...
sidebar_label: Hero
---
Introducing the Hero Component
 ...
 * Introducing the `svg` icon, it is a **graphical icon** which has made in use for the arrow in button. It's code is provided with the specifications of svg's `structures` and `attributes`. It is represented by the lines **11-25**.
 ...
 8. The `View more` element in this section, is programmed using the `<a> anchor` element. The CSS classes are applied to it including the **hover effect**. The anchor element here is representing the `hyperlink` which is redirecting to [*About Us*](https://tnp.tcetmumbai.in/about-us/).
    ...
    ```
 9. The  CountUpComponent displayed in the code handles the **counting animation** of numbers. There are props passed to `CountUpComponent`, which are as follows:
